...
def test_signals_when_add_folder_menu_item_clicked(driver):
    main_window = show_page(driver,
                            select_tracks_in_folder=lambda type_, on_select: on_select("track1." + type_,
                                                                                       "track2." + type_))
    album = build.album()
...
def test_signals_when_export_menu_item_clicked(driver):
    main_window = show_page(driver, select_export_destination=lambda on_select, name: on_select(name + ".csv"))
...
def test_signals_when_save_project_menu_item_clicked(driver):
    ...
    main_window = show_page(driver, on_save_album=save_album_signal.received)
    main_window.display_album_screen(album)
...
def test_signals_when_about_qt_menu_item_clicked(driver):
    ...
    show_page(driver, on_about_qt=lambda pressed: about_qt_signal.received())
...
def test_signals_when_about_menu_item_clicked(driver):
    ...
    show_page(driver, on_about=lambda pressed: about_signal.received())
...
def test_signals_when_online_help_menu_item_clicked(driver):
    ...
    show_page(driver, on_online_help=online_help_signal.received)
...
def test_signals_when_request_feature_menu_item_clicked(driver):
    ...
    show_page(driver, on_request_feature=request_feature_signal.received)
...
def test_signals_when_register_menu_item_clicked(driver):
    ...
    show_page(driver, on_register=register_signal.received)
...
def test_asks_for_confirmation_before_closing_project(driver):
    ...
    main_window = show_page(driver, confirm_close=lambda **_: confirm_close_query.received(), on_close_album=ignore)
    main_window.display_album_screen(album)
...
def test_warn_user_if_save_fails(driver):
    ...
    page = show_page(driver, on_save_album=_raise_os_error("Save failed"), show_save_error=save_failed_signal.received)
...
def test_warn_user_if_export_fails(driver):
    ...
    page = show_page(driver,
                     on_export=_raise_os_error("Export failed"), show_export_error=export_failed_signal.received,
...
def test_signals_when_sign_out_menu_item_clicked(driver):
    ...
    page = show_page(driver, on_sign_out=sign_out_signal.received)
    page.user_signed_in(User(email="test@example.com"))
...
def test_signals_when_transmit_to_soproq_menu_item_clicked(driver):
    ...
    page = show_page(driver,
                     select_save_as_destination=lambda on_select, name: on_select(name + ".xlsx"),
