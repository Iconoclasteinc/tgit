...
from test.integration.ui import ignore, show_, yes
from test.integration.ui import no
...
def test_signals_project_creation(driver, using_shortcut):
    show_page(of_type="flac", on_create_project=lambda *args: create_project_signal.received(args))
...
def test_selects_an_project_location(driver):
    _ = show_page(select_project_location=lambda handler: handler("/path/to/project"))
...
def test_selects_reference_track_location(driver):
    _ = show_page(select_track=lambda type_, handler: handler("track." + type_))
...
def test_asks_for_confirmation_when_project_file_already_exists(driver):
    ...
    _ = show_page(project_exists=lambda *args: project_exists_query.received(args))
...
def test_creates_project_if_confirmed(driver):
    ...
    _ = show_page(of_type="flac", project_exists=yes,
