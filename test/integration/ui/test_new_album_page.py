...
def test_selects_an_project_location(driver):
    show_page(select_project_location=lambda handler: handler("/path/to/project"))
...
def test_asks_for_confirmation_when_project_file_already_exists(driver):
    ...
    show_page(project_exists=lambda *args: project_exists_query.received(args))
...
def test_creates_project_if_confirmed(driver):
    ...
    show_page(of_type="flac", project_exists=yes, on_create_project=lambda *args: create_project_signal.received(args))
